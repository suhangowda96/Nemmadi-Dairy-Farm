...
const MedicineMonthlyStatus: React.FC<MedicineSummaryProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const calculateTotals = () => {
    return filteredRecords.reduce((total, record) => {
      return total + record.total_medicines;
    }, 0);
  };

  const totalMedicines = calculateTotals();
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `medicine_summary_${new Date().toISOString().slice(0, 10)}.xlsx`;
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Medicine Summary</h1>
              <p className="text-gray-600 mt-1">Monthly medicine administration records</p>
            </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Medicine Types</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dosage Summary</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Pill className="w-4 h-4 text-blue-400 mr-2" />
                            <span className="text-sm font-medium text-blue-600">
                              {record.total_medicines}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3 max-w-xs">
                          {record.medicine_types ? (
                            <div className="flex flex-wrap gap-1">
                              {record.medicine_types.split(', ').map((medicine: string) => (
                                <span 
                        ...
                        <td className="px-4 py-3 max-w-xs">
                          {record.dosage_summary ? (
                            <div className="flex flex-wrap gap-1">
                              {record.dosage_summary.split(', ').map((dosage: string) => (
                                <span 
                                  key={`${record.id}-${dosage}`} 
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-1 gap-4">
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Medicines</h3>
                      ...
                      <div className="flex flex-col">
                        <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Medicine Types</h3>
                        <div className="flex flex-wrap gap-1">
                          {record.medicine_types ? (
                            record.medicine_types.split(', ').map((medicine: string) => (
                              <span 
                      ...
                      <div className="flex flex-col">
                        <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Dosage Summary</h3>
                        <div className="flex flex-wrap gap-1">
                          {record.dosage_summary ? (
                            record.dosage_summary.split(', ').map((dosage: string) => (
                              <span 
...
export default MedicineMonthlyStatus;