...
import { 
  ...
  Search, Edit, X, Filter, ChevronDown, ChevronUp, Download, Trash2, Loader2
} from 'lucide-react';
...
const SATaskAssignment: React.FC<TaskAssignmentProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  // Fixed: Corrected endpoint for fetching employee data
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.length > 0) {
          setFormData(prev => ({ 
            ...prev, 
            staff_name: data[0].employee_name 
          }));
          setEmployeeError('');
        } else {
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      ...
      record.assigned_task?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.area_of_work?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.supervisor_remarks?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleEdit = (record: any) => {
    ...
    setFormData({
      ...
      area_of_work: record.area_of_work || '',
      completion_status: record.completion_status || 'Pending',
      supervisor_remarks: record.supervisor_remarks || ''
    });
    setShowForm(true);
  };
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `task_assignments_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this task assignment? This action cannot be undone.')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              ...
              <p className="text-gray-600 mt-1">Manage staff assignments and track completion status</p>
            </div>
            {showAddButton && (
              <button
            ...
            >
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit Task Assignment' : 'Add Task Assignment'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Area of Work *</label>
                      <input
                        type="text"
                        name="area_of_work"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Supervisor Remarks</label>
                    <textarea
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Task Assignments</h2>
            ...
            <>
                        ...
                        <td className="px-4 py-3 max-w-xs">
                          <div className="bg-gray-50 rounded p-2">
                            <p className="text-sm text-gray-900 break-words">
                              {record.assigned_task}
                            </p>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">{record.area_of_work}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            record.completion_status === 'Done' 
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-1 gap-4">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Area</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.area_of_work}
                          </p>
                        </div>
                    ...
                    {record.supervisor_remarks && (
                      <div className="p-4 border-t border-gray-100">
                        <h3 className="text-xs font-semibold text-gray-500 uppercase mb-2">Supervisor Remarks</h3>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No task assignments found</p>
                  <p className="text-gray-400 text-sm mb-4">Try adjusting your filters or add a new assignment</p>
                  <button
                    onClick={clearFilters}
...
export default SATaskAssignment;