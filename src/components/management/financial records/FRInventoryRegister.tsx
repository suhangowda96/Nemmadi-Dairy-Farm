...
interface FRInventoryRegisterProps {
  query?: string;
}

const FRInventoryRegister: React.FC<FRInventoryRegisterProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const calculateTotals = () => {
    ...
    filteredRecords.forEach(record => {
      totals.totalQuantity += parseFloat(record.quantity) || 0;
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `fr_inventory_register_${new Date().toISOString().slice(0, 10)}.xlsx`;
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Inventory Register</h1>
              <p className="text-gray-600 mt-1">Tracking of feed and inventory items</p>
            </div>
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by item, PO, or vendor..."
                  value={searchTerm}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-green-500">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{formatIndianNumber(totals.totalQuantity)} units</p>
            </div>
          </div>
            ...
            <>
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-purple-700">
                          <div className="flex items-center">
                            {getItemIcon(record.item_type)}
                            <span className="ml-2 capitalize">{record.item_type?.toLowerCase().replace('_', ' ') || 'N/A'}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-purple-700">
                          {record.quantity ? formatIndianNumber(parseFloat(record.quantity)) : 'N/A'}
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-700">
                    ...
                    <div className="p-4 grid grid-cols-1 gap-4">
                      ...
                      <div className="col-span-1">
                        <div className="flex items-start">
                          ...
                          <div className="ml-3">
                            <h3 className="text-xs font-semibold text-gray-500 uppercase">Item Type</h3>
                            <p className="text-sm font-medium capitalize">
                              {record.item_type?.toLowerCase().replace('_', ' ') || 'N/A'}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-span-1">
                        <div className="flex items-start">
                          ...
                          <div className="ml-3">
                            ...
                            <p className="text-sm font-medium">{record.item || 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-span-1">
                        <div className="flex items-start">
                          ...
                          <div className="ml-3">
                            <h3 className="text-xs font-semibold text-gray-500 uppercase">Vendor</h3>
                            <p className="text-sm font-medium">{record.vendor || 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex items-start">
                          ...
                          <div className="ml-3">
                            ...
                            <p className="text-sm font-medium">
                              {record.quantity ? `${formatIndianNumber(parseFloat(record.quantity))} units` : 'N/A'}
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No inventory records found</p>
...
export default FRInventoryRegister;