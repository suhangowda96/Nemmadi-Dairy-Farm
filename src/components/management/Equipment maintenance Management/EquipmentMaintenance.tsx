...
interface EMEquipmentMaintenanceProps {
  query?: string;
}

const EquipmentMaintenance : React.FC<EMEquipmentMaintenanceProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const activeParts = data.filter((r: any) => r.state === 'active').length;
        const removedParts = data.filter((r: any) => r.state === 'removed').length;
        const totalCost = data.reduce((sum: number, r: any) => sum + (r.total_cost || 0), 0);
        const averageCost = totalParts > 0 ? totalCost / totalParts : 0;
  ...
  const filteredRecords = records.filter(record => {
    ...
    const matchesPart = partFilter === '' || 
      record.part_name === partFilter;
    
    const matchesVendor = vendorFilter === '' || 
      record.vendor === vendorFilter;
    
    return matchesSearch && matchesDate && matchesPart && matchesVendor;
  });
    ...
    try {
      ...
      const dataForExport = filteredRecords.map(record => {
        return {
          ...
          'Cost per Part': record.cost_per_equipment,
      ...
      XLSX.writeFile(wb, `spareparts_inventory_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Spare Parts Inventory</h1>
              <p className="text-gray-600 mt-1">Manage farm equipment parts inventory</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-blue-500">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{formatIndianNumber(summary.totalParts)}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-green-500">
            ...
            <div>
              <p className="text-xs text-gray-500">Active Parts</p>
              <p className="text-lg font-bold">{formatIndianNumber(summary.activeParts)}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-red-500">
            ...
            <div>
              <p className="text-xs text-gray-500">Removed Parts</p>
              <p className="text-lg font-bold">{formatIndianNumber(summary.removedParts)}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-yellow-500">
            ...
            <div>
              <p className="text-xs text-gray-500">Avg Cost/Part</p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredRecords.map((record) => {
                      ...
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">
                            ₹{formatCurrency(record.cost_per_equipment || 0)}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm font-medium text-gray-900">
