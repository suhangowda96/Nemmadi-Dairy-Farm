...
import { 
  Plus, Calendar, Settings, FileText, Phone, CalendarCheck, 
  Search, Edit, X, Filter, ChevronDown, ChevronUp, Download, Trash2, CheckCircle, XCircle
} from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { useAuth } from '../../../contexts/AuthContext';

interface ServiceAMCProps {
  query?: string;
}

const EMServiceAMC: React.FC<ServiceAMCProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState({
    ...
    equipment_name: '',
    amc: amcChoices[0].value,
    vendor_service_agency: '',
    contract_period: '',
    contact_number: '',
    last_service_date: '',
    next_due_date: '',
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      ...
      record.vendor_service_agency?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEdit = (record: any) => {
    ...
    setFormData({
      ...
      amc: record.amc,
      vendor_service_agency: record.vendor_service_agency,
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `amc_records_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this AMC record? This action cannot be undone.')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Service AMC Records</h1>
              <p className="text-gray-600 mt-1">Manage Annual Maintenance Contracts for equipment</p>
            </div>
            {showAddButton && (
              <button
              ...
              >
                ...
                <span className="font-medium">Add AMC Record</span>
              </button>
            )}
          </div>
        </div>
            ...
            >
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit AMC Record' : 'Add AMC Record'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="flex space-x-4 mt-2">
                        {amcChoices.map((choice) => (
                          <label key={choice.value} className="flex items-center">
                            <input
                              type="radio"
                              name="amc"
                              value={choice.value}
                              checked={formData.amc === choice.value}
                              onChange={handleChange}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Vendor/Service Agency *</label>
                      <input
                        type="text"
                        name="vendor_service_agency"
                        value={formData.vendor_service_agency}
                        onChange={handleChange}
                        required
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      rows={3}
                      placeholder="Additional notes about the AMC"
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">AMC Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">AMC</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Vendor</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Contract Period</th>
                      ...
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-gray-900">{record.equipment_name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            {record.amc === 'Y' ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">{record.vendor_service_agency}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">{record.contract_period}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Phone className="w-4 h-4 text-gray-500 mr-2" />
                            <span className="text-sm text-gray-900">{record.contact_number}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">
                            {new Date(record.last_service_date).toLocaleDateString('en-GB')}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <CalendarCheck className="w-4 h-4 text-green-500 mr-2" />
                            <span className="text-sm font-medium text-gray-900">
                    ...
                    <div className="p-4 grid grid-cols-1 gap-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{record.vendor_service_agency}</p>
                        </div>
                        <div>
                          <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Contract Period</h3>
                          <p className="text-sm font-medium text-gray-900">{record.contract_period}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex items-center">
                            ...
                            <p className="text-sm font-medium text-gray-900">{record.contact_number}</p>
                          </div>
                        </div>
                        <div>
                          <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Last Service</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {new Date(record.last_service_date).toLocaleDateString('en-GB')}
                          </p>
                        </div>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No AMC records found</p>
