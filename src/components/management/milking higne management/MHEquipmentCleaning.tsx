...
import { 
  Plus, Calendar, ClipboardList, CheckCircle2, XCircle, 
  Search, Edit, X, Filter, ChevronDown, ChevronUp, 
  Download, Trash2, Droplets, Clock, User
} from 'lucide-react';
...
interface EquipmentCleaningProps {
  query?: string;
}

const MHEquipmentCleaning: React.FC<EquipmentCleaningProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `equipment_cleaning_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this cleaning record? This action cannot be undone.')) {
      try {
        setDeletingId(id);
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Equipment Cleaning Records</h1>
            </div>
            {showAddButton && (
            <button
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search equipment or remarks..."
                  value={searchTerm}
            ...
            >
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Cleaned *</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Sanitizer Used *</label>
                      <input
                        type="text"
                        name="sanitizer_used"
                        value={formData.sanitizer_used}
                        onChange={handleChange}
                        required
                        placeholder="Enter sanitizer name"
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Cleaning Time *</label>
                      <input
                        type="time"
                        name="cleaning_time"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      rows={3}
                      placeholder="Additional notes about the cleaning process"
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sanitizer</th>
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <ClipboardList className="w-4 h-4 text-blue-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.equipment_name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-900">{record.sanitizer_used}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-900">
                              {formatTime(record.cleaning_time)}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          ...
                          <p className={`text-sm font-medium ${record.cleaned === 'Y' ? 'text-green-700' : 'text-red-700'}`}>
                            {record.cleaned === 'Y' ? 'Yes' : 'No'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Sanitizer</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.sanitizer_used}
                          </p>
                        </div>
                      </div>
...
export default MHEquipmentCleaning;