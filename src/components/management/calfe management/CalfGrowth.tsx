...
import { 
  ...
  Scale, Activity, Heart, Tag, Utensils,
} from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { useAuth } from '../../../contexts/AuthContext';

interface CalfGrowthRecord {
  id: number;
  calf_id: string;
  date: string;
  weight: number;
...
interface CalfGrowthProps {
  query?: string;
}

const CalfGrowth: React.FC<CalfGrowthProps> = ({ query = '' }) => {
  const { user } = useAuth();
  const [showForm, setShowForm] = useState(false);
  const [showFilters, setShowFilters] = useState(false);
  const [editingRecord, setEditingRecord] = useState<CalfGrowthRecord | null>(null);
  const [records, setRecords] = useState<CalfGrowthRecord[]>([]);
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      ...
      record.body_condition?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.feeding_behavior?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEdit = (record: CalfGrowthRecord) => {
    ...
    setFormData({
      ...
      feeding_behavior: record.feeding_behavior,
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `calf_growth_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this calf growth record? This action cannot be undone.')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Calf Growth Monitoring</h1>
              <p className="text-gray-600 mt-1">Track growth metrics and health status of calves</p>
            </div>
            {showAddButton && (
              <button
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by calf ID, condition, or behavior..."
                  value={searchTerm}
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Feeding Behavior *</label>
                      <select
                        name="feeding_behavior"
                        value={formData.feeding_behavior}
                        onChange={handleChange}
                        required
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      rows={3}
                      placeholder="Additional observations about the calf..."
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Growth Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Body Condition</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Feeding Behavior</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Health Status</th>
                      ...
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-900 font-medium">{record.weight}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusStyle(record.body_condition)}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Utensils className="w-4 h-4 text-green-500 mr-2" />
                            <span className="text-sm text-gray-900">{record.feeding_behavior}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Heart className="w-4 h-4 text-red-500 mr-2" />
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusStyle(record.health_status)}`}>
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Body Condition</h3>
                          <p className={`text-sm font-medium ${getStatusStyle(record.body_condition)} px-2 py-1 rounded-full`}>
                            {record.body_condition}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start">
                        <Utensils className="w-5 h-5 text-blue-500 mt-0.5 flex-shrink-0" />
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Feeding Behavior</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.feeding_behavior}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start col-span-2">
                        ...
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Health Status</h3>
                          <p className={`text-sm font-medium ${getStatusStyle(record.health_status)} px-2 py-1 rounded-full`}>
                            {record.health_status}
                          </p>
                        </div>
                      </div>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No growth records found</p>
