...
import { 
  ...
  Tag, User, Scale, Baby, GitMerge
} from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { useAuth } from '../../../contexts/AuthContext';

interface CalfIdentificationRecord {
  id: number;
  calf_id: string;
...
interface CalfIdentificationProps {
  query?: string;
}

const CalfIdentification: React.FC<CalfIdentificationProps> = ({ query = '' }) => {
  const { user } = useAuth();
  const [showForm, setShowForm] = useState(false);
  const [showFilters, setShowFilters] = useState(false);
  const [editingRecord, setEditingRecord] = useState<CalfIdentificationRecord | null>(null);
  const [records, setRecords] = useState<CalfIdentificationRecord[]>([]);
...
useEffect(() => {
  const fetchCalves = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Extract all calf IDs (both active and inactive)
        const allCalves = data.map((record: any) => record.calf_id);
        setCalfOptions(allCalves);
      }
    } catch (err) {
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      ...
      record.dam_id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.sire_id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEdit = (record: CalfIdentificationRecord) => {
    ...
    setFormData({
      ...
      breed: record.breed,
      birth_weight: record.birth_weight.toString(),
      calving_type: record.calving_type,
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `calf_identification_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this calf record? This action cannot be undone.')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Calf Identification</h1>
              <p className="text-gray-600 mt-1">Track and manage calf identification records</p>
            </div>
            {showAddButton && (
              <button
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by calf ID, dam ID, sire ID, or breed..."
                  value={searchTerm}
              ...
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit Calf Record' : 'Add New Calf'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Dam ID *</label>
                      <input
                        type="text"
                        name="dam_id"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Sire ID *</label>
                      <input
                        type="text"
                        name="sire_id"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Calving Type *</label>
                      <input
                        name="calving_type"
                        value={formData.calving_type}
                        onChange={handleChange}
                        required
                        placeholder="Calving Type"
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Calf Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dam ID</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sire ID</th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Calving Type</th>
                      ...
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-900">{record.dam_id}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <User className="w-4 h-4 text-indigo-500 mr-2" />
                            <span className="text-sm text-gray-900">{record.sire_id}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <GitMerge className="w-4 h-4 text-green-500 mr-2" />
                            <span className="text-sm text-gray-900">{record.breed}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Scale className="w-4 h-4 text-yellow-500 mr-2" />
                            <span className="text-sm text-gray-900">{record.birth_weight}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Baby className="w-4 h-4 text-cyan-500 mr-2" />
                            <span className="text-sm text-gray-900 capitalize">{record.calving_type}</span>
                          </div>
                        </td>
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">DOB</h3>
                      ...
                      <div className="flex items-start">
                        <span className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full ${
                          record.gender === 'Male' ? 'bg-blue-500' : 'bg-pink-500'
                        }`}></span>
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Gender</h3>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Dam ID</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.dam_id}
                          </p>
                        </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Sire ID</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.sire_id}
                          </p>
                        </div>
                      ...
                      <div className="flex items-start col-span-2">
                        <GitMerge className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" />
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Breed</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.breed}
                          </p>
                        </div>
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4 border-t border-gray-100">
                      <div className="flex items-start">
                        <Scale className="w-5 h-5 text-yellow-500 mt-0.5 flex-shrink-0" />
                        <div className="ml-2">
                      ...
                      <div className="flex items-start">
                        <Baby className="w-5 h-5 text-cyan-500 mt-0.5 flex-shrink-0" />
                        <div className="ml-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Calving Type</h3>
                          <p className="text-sm font-medium text-gray-900 capitalize">
                            {record.calving_type}
                          </p>
                        </div>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm mb-4">Try adjusting your filters or add a new calf record</p>
                  <button
                    onClick={clearFilters}
...
export default CalfIdentification;