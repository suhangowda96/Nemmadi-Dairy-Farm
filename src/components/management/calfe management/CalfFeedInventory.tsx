...
import { 
  ...
  Package, Scale, TrendingUp, Milk, Baby, Wheat,PiggyBank
} from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { useAuth } from '../../../contexts/AuthContext';

interface CalfFeedInventoryProps {
  query?: string;
}
...
// Calf feed type options
const CALF_FEED_TYPES = [
  'Milk Replacer',
  'Calf Starter',
  'Grower Ration',
  'Heifer'
...
// Icon mapping for feed types
const FEED_TYPE_ICONS: Record<string, React.ReactNode> = {
  'Milk Replacer': <Milk className="w-5 h-5 text-blue-500" />,
  'Calf Starter': <Baby className="w-5 h-5 text-green-500" />,
  'Grower Ration': <Wheat className="w-5 h-5 text-amber-500" />,
  'Heifer': <PiggyBank className="w-5 h-5 text-purple-500" />
};

const CalfFeedInventory: React.FC<CalfFeedInventoryProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `calf_feed_inventory_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Calf Feed Inventory</h1>
              <p className="text-gray-600 mt-1">Monthly calf feed stock management and tracking</p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 flex items-center space-x-3 border-l-4 border-yellow-500">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{formatIndianNumber(totals.totalBalance)} kg</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Calf Feed Inventory Records</h2>
              <div className="flex items-center space-x-3">
            ...
            <>
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            {FEED_TYPE_ICONS[record.feed_type]}
                        ...
                        <td className="px-4 py-3 text-sm font-medium">
                          <span className={`px-2 py-1 rounded-full ${record.balance_kg < 10 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-blue-700">
                          ₹{formatIndianNumber(record.cost_per_kg)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No calf feed records found</p>
...
export default CalfFeedInventory;