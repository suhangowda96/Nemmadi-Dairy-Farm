...
import { 
  ...
  Skull, IndianRupee, Heart, Activity, 
} from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { useAuth } from '../../../contexts/AuthContext';

interface DeathOrSoldRecord {
  id: number;
  calf_id: string;
  date: string;
  event_type: 'Death' | 'Sold';
  cost: number | null;
  reason: string | null;
  remarks: string | null;
  created_at: string;
...
interface DeathOrSoldLogProps {
  query?: string;
}

const CalvingDeathOrSoldLog: React.FC<DeathOrSoldLogProps> = ({ query = '' }) => {
  const { user } = useAuth();
  const [showForm, setShowForm] = useState(false);
  const [showFilters, setShowFilters] = useState(false);
  const [editingRecord, setEditingRecord] = useState<DeathOrSoldRecord | null>(null);
  const [records, setRecords] = useState<DeathOrSoldRecord[]>([]);
  ...
  const [formData, setFormData] = useState({
    calf_id: '',
    date: '',
    event_type: 'Death' as 'Death' | 'Sold',
    cost: '',
    reason: '',
    remarks: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only include cost/reason based on event type
      if (formData.event_type === 'Sold') {
        payload.cost = parseFloat(formData.cost);
      } else if (formData.event_type === 'Death') {
  ...
  const handleEdit = (record: DeathOrSoldRecord) => {
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `death_sold_records_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Calf Exit Records</h1>
              <p className="text-gray-600 mt-1">Track calf death and sale events</p>
            </div>
            {showAddButton && (
              <button
              ...
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit Record' : 'Add Calf Exit Record'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
                      ...
                      <div className="mt-1">
                        <label className="inline-flex items-center mr-6">
                          <input
                            type="radio"
                            name="event_type"
                            checked={formData.event_type === 'Death'}
                            onChange={() => setFormData(prev => ({ ...prev, event_type: 'Death' }))}
                            className="form-radio h-4 w-4 text-blue-600"
                          />
                          <span className="ml-2 text-gray-700 flex items-center">
                            <Skull className="w-4 h-4 mr-1" /> Death
                          </span>
                        </label>
                        <label className="inline-flex items-center">
                          <input
                            type="radio"
                            name="event_type"
                            checked={formData.event_type === 'Sold'}
                            onChange={() => setFormData(prev => ({ ...prev, event_type: 'Sold' }))}
                            className="form-radio h-4 w-4 text-blue-600"
                          />
                          <span className="ml-2 text-gray-700 flex items-center">
                            <IndianRupee className="w-4 h-4 mr-1" /> Sold
                          </span>
                        </label>
                      </div>
                    </div>
                    
                    {formData.event_type === 'Sold' ? (
                      <div>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Death Reason *</label>
                        <input
                          type="text"
                          name="reason"
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Calf Exit Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Death Reason</th>
                ...
                {filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      ...
                      {record.event_type === 'Sold' ? (
                        <div className="flex items-start">
                          <IndianRupee className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" />
                          <div className="ml-2">
                            <h3 className="text-xs font-semibold text-gray-500 uppercase">Sale Price</h3>
                            <p className="text-sm font-medium text-gray-900">
                              {formatIndianCurrency(record.cost)}
                            </p>
                          </div>
                        ...
                        <div className="flex items-start">
                          ...
                          <div className="ml-2">
                            <h3 className="text-xs font-semibold text-gray-500 uppercase">Death Reason</h3>
                            <p className="text-sm font-medium text-gray-900">
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No exit records found</p>
...
export default CalvingDeathOrSoldLog;