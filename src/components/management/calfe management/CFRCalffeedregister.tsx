...
interface CFRCalffeedregisterProps {
  query?: string;
}

const CFRCalffeedregister: React.FC<CFRCalffeedregisterProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError('Failed to fetch calf feed records');
      }
    } catch (err) {
  ...
  const calculateTotals = () => {
    ...
    filteredRecords.forEach(record => {
      totals.totalStock += parseFloat(record.stock_received) || 0;
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `calf_feed_register_${new Date().toISOString().slice(0, 10)}.xlsx`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        window.URL.revokeObjectURL(downloadUrl);
      } else {
        setError('Failed to export calf feed data');
      }
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Calf Feed Register</h1>
              <p className="text-gray-600 mt-1">Tracking of calf feed stock received and quality</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Calf Feed Register Records</h2>
              <div className="flex items-center space-x-3">
          ...
          {isLoading ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Loading calf feed records...</p>
            </div>
          ) : error ? (
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-blue-700">
                          {record.stock_received ? formatIndianNumber(parseFloat(record.stock_received), 2) : 'N/A'}
                        </td>
                        ...
                        <div className="flex items-start">
                          ...
                          <div className="ml-3">
                            ...
                            <p className="text-sm font-medium">
                              {record.stock_received ? `${formatIndianNumber(parseFloat(record.stock_received), 2)} kg` : 'N/A'}
                            </p>
                          </div>
                        </div>
                      </div>
...
export default CFRCalffeedregister;