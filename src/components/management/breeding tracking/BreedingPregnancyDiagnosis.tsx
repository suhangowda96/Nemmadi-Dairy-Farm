...
import { 
  Plus, Calendar, Heart, Search, Edit, X, Filter, 
  ChevronDown, ChevronUp, Download, Trash2, Check, XCircle
} from 'lucide-react';
...
interface PregnancyDiagnosisProps {
  query?: string;
}

const BreedingPregnancyDiagnosis: React.FC<PregnancyDiagnosisProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const filteredRecords = records.filter(record => {
    const recordDate = new Date(record.pd_date);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedDate = formData.pd_date.toISOString().split('T')[0];
  ...
  const handleEdit = (record: any) => {
    ...
    setFormData({
      ...
      pd_date: new Date(record.pd_date),
      pd_status: record.pd_status,
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `pregnancy_diagnoses_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this pregnancy diagnosis record?')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Pregnancy Diagnosis</h1>
            </div>
            {showAddButton && (
            <button
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by animal ID, vet, or remarks..."
                  value={searchTerm}
              ...
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit Pregnancy Diagnosis' : 'Add Pregnancy Diagnosis'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Pregnancy Diagnosis Records</h2>
            ...
            <>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-gray-900">
                              {new Date(record.pd_date).toLocaleDateString('en-GB')}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {getStatusDisplay(record.pd_status)}
                        </td>
                        <td className="px-4 py-3">
                ...
                {filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      ...
                      <div className="col-span-2">
                        <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Pregnancy Status</h3>
                        <div className="mt-1">
                          {getStatusDisplay(record.pd_status)}
                        </div>
                      </div>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No pregnancy diagnosis records found</p>
                  <p className="text-gray-400 text-sm mb-4">Try adjusting your filters or add a new diagnosis</p>
                  <button
                    onClick={clearFilters}
...
export default BreedingPregnancyDiagnosis;