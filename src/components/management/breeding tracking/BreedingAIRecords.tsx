...
import { 
  Plus, Calendar, Search, Edit, X, Filter, ChevronDown, ChevronUp, 
  ClipboardList, Download, Trash2, Syringe, User, Check, XCircle
} from 'lucide-react';
...
const BreedingAIRecords: React.FC<BreedingRecordProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState({
    date_of_ai: new Date().toISOString().split('T')[0],
    animal_id: '',
    semen_code: '',
    bull_breed: '',
    ai_technician: '',
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      ...
      record.semen_code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.ai_technician?.toLowerCase().includes(searchTerm.toLowerCase());
    
    const recordDate = new Date(record.date_of_ai);
  ...
  const handleEdit = (record: any) => {
    ...
    setFormData({
      date_of_ai: record.date_of_ai,
      animal_id: record.animal_id,
      semen_code: record.semen_code,
      bull_breed: record.bull_breed,
      ai_technician: record.ai_technician,
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `AI_records_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">AI Records</h1>
            </div>
            {showAddButton && (
              <button
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search animal ID, semen code, technician..."
                  value={searchTerm}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        type="date"
                        name="date_of_ai"
                        value={formData.date_of_ai}
                        onChange={e => setFormData({...formData, date_of_ai: e.target.value})}
                        required
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                ...
                <div className="relative" ref={animalDropdownRef}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setFormData({...formData, semen_code: e.target.value})}
                        required
                        placeholder="Semen identification code"
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setFormData({...formData, bull_breed: e.target.value})}
                        required
                        placeholder="Breed of the bull"
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setFormData({...formData, ai_technician: e.target.value})}
                        required
                    ...
                    <div className="flex space-x-4">
                      <label className="flex items-center">
                        <input
                          ...
                          name="success"
                          value="Y"
                          checked={formData.success === 'Y'}
                          onChange={() => setFormData({...formData, success: 'Y'})}
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          name="success"
                          value="N"
                          checked={formData.success === 'N'}
                          onChange={() => setFormData({...formData, success: 'N'})}
                          className="h-4 w-4 text-blue-600"
                        />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Additional notes about the AI procedure"
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">AI Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Semen Code</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bull Breed</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Syringe className="w-4 h-4 text-purple-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.semen_code}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-gray-900">{record.bull_breed}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <User className="w-4 h-4 text-green-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.ai_technician}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          {record.success === 'Y' ? (
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                              <Check className="w-4 h-4 mr-1" /> Successful
                            </span>
                          ) : (
                ...
                {filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4">
                      ...
                      <div className="flex items-start">
                        <Syringe className="w-5 h-5 text-purple-400 mt-0.5 flex-shrink-0" />
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Semen Code</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.semen_code}
                          </p>
                        </div>
                      ...
                      <div className="col-span-2 flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Technician</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.ai_technician}
                          </p>
                        </div>
                      </div>
                      
                      <div>
                        <h3 className="text-xs font-semibold text-gray-500 uppercase">Bull Breed</h3>
                        <p className="text-sm font-medium text-gray-900">
                          {record.bull_breed}
                        </p>
                      </div>
                      ...
                      <div>
                        <h3 className="text-xs font-semibold text-gray-500 uppercase">Success</h3>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No AI records found</p>
...
export default BreedingAIRecords;