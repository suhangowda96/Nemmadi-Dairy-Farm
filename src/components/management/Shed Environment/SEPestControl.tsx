...
import { 
  ...
  ClipboardList, Download, Trash2, Bug, SprayCan, User, FlaskConical 
} from 'lucide-react';
...
const formatTime = (timeString: string) => {
  ...
  const hour12 = hourNum % 12 || 12; // Convert 0 to 12 for AM
...
const PestControl: React.FC<PestControlProps> = ({ query = '' }) => {
  const { user } = useAuth();
  ...
  const filteredRecords = records.filter(record => {
    const matchesSearch = searchTerm === '' || 
      record.area_sprayed?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.product_used?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEdit = (record: any) => {
    ...
    setFormData({
      date: record.date,
      area_sprayed: record.area_sprayed,
      method_used: record.method_used,
      time: record.time.split('T')[1].substring(0, 5),
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this pest control record? This action cannot be undone.')) {
      try {
        setDeletingId(id);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Pest Control</h1>
            </div>
            {showAddButton && (
            <button
            ...
            >
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 lg:p-6 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-bold text-gray-900">
                    {editingRecord ? 'Edit Pest Control Record' : 'Add Pest Control Record'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowForm(false);
              ...
              <div className="p-4 lg:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Area Sprayed *</label>
                    <input
                      type="text"
                      name="area_sprayed"
                      value={formData.area_sprayed}
                      onChange={handleChange}
                      required
                      placeholder="Area sprayed"
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    />
                  </div>
                      ...
                      {METHOD_CHOICES.map((method) => (
                        <div key={method.value} className="flex items-center">
                          <input
                            ...
                            name="method_used"
                            value={method.value}
                            checked={formData.method_used === method.value}
                            onChange={handleChange}
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                          />
                          <label htmlFor={`method-${method.value}`} className="ml-3 block text-sm font-medium text-gray-700">
                            {method.label}
                          </label>
                        </div>
                      ))}
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <h2 className="text-xl font-bold text-gray-900">Pest Control Records</h2>
            ...
            <>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Area Sprayed</th>
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <Bug className="w-4 h-4 text-orange-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.area_sprayed}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <SprayCan className="w-4 h-4 text-blue-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.method_used}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <FlaskConical className="w-4 h-4 text-purple-400 mr-2" />
                            <span className="text-sm text-gray-900">{record.product_used}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                  ...
                  >
                    ...
                    <div className="p-4 grid grid-cols-1 gap-4">
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Area Sprayed</h3>
                      ...
                      <div className="flex items-start">
                        <SprayCan className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
                        <div className="ml-3">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-3">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase">Product</h3>
                          <p className="text-sm font-medium text-gray-900">
                            {record.product_used}
                          </p>
                        </div>
                      ...
                      <div className="flex items-start">
                        <span className="text-xs font-semibold text-gray-500 uppercase mt-0.5">Time:</span>
                        <div className="ml-3">
                          <p className="text-sm font-medium text-gray-900">
                            {formatTime(record.time)}
                          </p>
              ...
              {filteredRecords.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg mb-2">No pest control records found</p>
